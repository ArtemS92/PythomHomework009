    ...
    def make_move(self, row, col):
        if self.end_of_game and not self.first_move:
            ...
            if self.field[0].count(self.field[0][0]) == 3 and self.field[0][0] != '-':
                ...
                self.win = self.field[2][0]
                return f'Победил игрок {self.field[2][0]}'
            elif self.field[0].count('X' or '0') != 3 and self.field[0].count('-') == 0 and \
                    self.field[1].count('X' or '0') != 3 and self.field[1].count('-') == 0 and \
                    self.field[2].count('X' or '0') != 3 and self.field[2].count('-') == 0:
                self.end_of_game = True
                self.win = 'ничья'
                return 'Ничья'
            else:
